...
(function() {
    ...
    // =====================================================
    const CONFIG = {
        // TopoJSON source – EU-only local file (generated by: node scripts/build-map.js)
        TOPOJSON_URL: 'data/europe.topojson',
    ...
    function updateViewBox() {
        ...
        _ctmCacheInvalid = true; // viewBox changes affect screen↔SVG mapping
    }
    ...
    function scatterPieces() {
        ...
        function overlapsOwnSilhouette(cluster, tx, ty) {
            ...
            const countryId = Array.from(cluster.members)[0];
            const country = state.countries.get(countryId);
            if (!country || !country.bounds) return false;
            const [[minX, minY], [maxX, maxY]] = country.bounds;
            // Translated piece bounds vs. natural silhouette bounds
            return (minX + tx) < maxX && (maxX + tx) > minX &&
                   (minY + ty) < maxY && (maxY + ty) > minY;
        }
            ...
            if (!placed) {
                ...
                if (state.difficulty === 'easy') {
                    ...
                    if (country && country.bounds) {
                        ...
                        const bw = maxX - minX;
                        const bh = maxY - minY;
                        // Keep nudging until clear (max 4 steps)
                        for (let s = 0; s < 4 && overlapsOwnSilhouette(cluster, x, y); s++) {
                            // Push away from board centre along whichever axis has smaller clearance
        ...
        if (state.difficulty === 'easy') {
            ...
            if (dist < CONFIG.SNAP_THRESHOLD) {
                ...
                animateSnap(draggedCluster, { x: 0, y: 0, rotation: 0 }, () => {
                    ...
                    // Push solved piece behind all unplaced pieces in SVG render order
                    state.clustersContainer.insertBefore(draggedCluster.element, state.clustersContainer.firstChild);
    ...
    function onPointerMove(e) {
        ...
        // Batch DOM writes to the next animation frame to avoid layout thrashing
        if (!state._dragRafPending) {
            state._dragRafPending = true;
            requestAnimationFrame(() => {
                state._dragRafPending = false;
                const c = state.clusters.get(state.selectedCluster);
                if (!c) return;
                updateClusterTransform(c.element, c.transform);
    ...
    function getSVGPoint(e) {
        if (_ctmCacheInvalid || !_cachedCTMInverse) {
            _cachedCTMInverse = state.svg.getScreenCTM().inverse();
            _ctmCacheInvalid  = false;
        }
        ...
        function drawFrame(now) {
            ...
            if (p >= 1) {
                ...
                if (country && !cluster.isSolved) {
                    ...
                    // Push solved piece behind all unplaced pieces in SVG render order
                    state.clustersContainer.insertBefore(cluster.element, state.clustersContainer.firstChild);
                    // Flash
                    paths.forEach(p => {
